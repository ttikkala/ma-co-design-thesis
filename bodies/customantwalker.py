# This file takes the given xml file and creates the Ant walkers in the dm_control simulator
# Modified from ant.py of the dm_control simulator, which is licensed under the following terms:
...
_XML_DIRNAME = "/home/tiia/thesis/bodies"
_XML_FILENAME = "ant_highres.xml"

class Ant(legacy_base.Walker):
  """A quadruped "Ant" walker."""

  def __init__(self, xml_filename=None, **kwargs):
    """Initialize the Ant walker.
    
    Args:
      xml_filename: (Optional) Name of the XML file in _XML_DIRNAME to use.
                   If None, uses the default ant_orig.xml.
      **kwargs: Additional arguments passed to _build().
    """
    self._xml_filename = xml_filename if xml_filename is not None else _XML_FILENAME
  ...
  def _build(self, name='walker', marker_rgba=None, initializer=None):
    ...
    self._mjcf_root = mjcf.from_path(os.path.join(_XML_DIRNAME, self._xml_filename))
    if name:
