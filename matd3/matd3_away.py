...
""" 
    This script contains the MATD3Away class which trains the actors and critics of a team of MATD3 agents playing for the away team.
"""
...
class MATD3Away():
    ...
    def train(self, replay, train_iters, batch_size = 64, obs_dim = 304):
        stats = []
        ...
        for iter in range(train_iters):
            ...
            for i in range(2, 4): # i is first 2 and then 3
                # initialise stuff
                ...
                for j in range(2, 4):
                    ...
                    target_action = torch.clamp(self.target_policy_nets[j - self.n_agents](local_states_j) + action_noise, -1.0, 1.0)
                    target_actions.append(target_action)
                
                home_team_actions = actions[:, 0:(self.n_agents * action_dim)]

                target_actions = torch.cat([home_team_actions] + target_actions, dim=1)


                # calculate value loss
                q_loss_fn = torch.nn.MSELoss()
                q1 = self.q1_nets[i - self.n_agents](normed_states, actions)
                q2 = self.q2_nets[i - self.n_agents](normed_states, actions)
                target_q1 = self.target_q1_nets[i - self.n_agents](normed_states_t, target_actions)
                target_q2 = self.target_q2_nets[i - self.n_agents](normed_states_t, target_actions)
                y = rewards[:,i - self.n_agents].view(-1, 1) + self.discount * torch.min(target_q1, target_q2)
                q_loss = q_loss_fn(y, q1) + q_loss_fn(y, q2)

                # optimiser step
                self.q_optimizers[i - self.n_agents].zero_grad()
                q_loss.backward()
                torch.nn.utils.clip_grad_norm_(self.q1_nets[i - self.n_agents].parameters(), max_norm=1.0)
                torch.nn.utils.clip_grad_norm_(self.q2_nets[i - self.n_agents].parameters(), max_norm=1.0)
                self.q_optimizers[i - self.n_agents].step()
                ...
                if iter % self.policy_delay == 0:
                    ...
                    for j in range(2, 4):
                        if j == i:
                            curr_actions.append(self.policy_nets[i - self.n_agents](local_states_i))
                        else:
                            curr_actions.append(actions[:, (j * action_dim):((j+1) * action_dim)])
                    
                    curr_actions = torch.cat([home_team_actions] + curr_actions, dim = 1)

                    # calculate policy loss
                    policy_loss = - (self.q1_nets[i - self.n_agents](normed_states, curr_actions)).mean()

                    # optimiser step
                    self.policy_optimizers[i - self.n_agents].zero_grad()
                    policy_loss.backward()
                    torch.nn.utils.clip_grad_norm_(self.policy_nets[i - self.n_agents].parameters(), max_norm=1.0)
                    self.policy_optimizers[i - self.n_agents].step()
                ...
                if iter % 400 == 0:
                    ...
                    iter_agent_stats[f'agent_{i - self.n_agents}'] = agent_stats

            if iter % self.policy_delay == 0:
                # update target networks
                for i in range(self.n_agents): # nb this is fine for away team as well
